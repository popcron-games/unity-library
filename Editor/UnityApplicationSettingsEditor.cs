...
namespace UnityLibrary.Unity
{
    ...
    public class UnityApplicationSettingsEditor : Editor
    {
        ...
        private void ValidateStateType()
        {
            ...
            TypeCache.TypeCollection types = TypeCache.GetTypesDerivedFrom<VirtualMachine.IState>();
            if (stateType is null)
            {
                ...
                if (string.IsNullOrEmpty(typeName))
                {
                    if (types.Count == 0)
                    {
                        EditorGUILayout.HelpBox($"No state type assigned. Please create a class that implements {typeof(VirtualMachine.IState).FullName} and assign it here.", MessageType.Error);
                    }
                    else
                    {
